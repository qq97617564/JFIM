...
/// 重写数据转换，增加completion回调
+ (void)tansformDataFromSqlDataWithTableName:(NSString *)tableName class:(__unsafe_unretained Class)cla array:(NSArray *)array completion:(void (^)(NSArray *))completion
{
    //如果传入的class为空，则直接以字典的形式返回.
    if(cla == nil){
        return;
    ...
    NSInteger count = array.count; // 数据总量
    NSMutableArray* arrM = [NSMutableArray arrayWithCapacity:count];
    NSInteger limit = 100; // 预设每多少条数据，开启一个新的任务
    
    
    // 计算任务量
    NSInteger taskCount = count / limit + (count%limit>0?1:0);
    
    //建立任务组
    dispatch_group_t group = dispatch_group_create();
    //设置信号量大小
    dispatch_semaphore_t semaphore = dispatch_semaphore_create(taskCount>10?10:taskCount);
    //创建全局队列
    ...
    // 添加任务
     for (int i = 0; i < taskCount; i++) {
          ...
          dispatch_group_async(group, queue, ^{
              ...
              NSInteger length = limit;
              if (i == taskCount-1) {
                  length  = count % limit;
              }

              for (NSMutableDictionary* dict in [array subarrayWithRange:NSMakeRange(i*limit, length)]) {
                  id object = [BGTool objectFromJsonStringWithTableName:tableName class:cla valueDict:dict];
                  [arrM addObject:object];
              }
    ...
    [[BGDB shareManager] isExistWithTableName:tableName complete:^(BOOL isExist) {
        if (!isExist){//如果不存在就新建
            ...
            [[BGDB shareManager] createTableWithTableName:tableName keys:createKeys unionPrimaryKeys:unionPrimaryKeys uniqueKeys:uniqueKeys complete:^(BOOL isSuccess) {
                ...
                if (BGDB.shareManager.createdDBBlock) {
                    BGDB.shareManager.createdDBBlock(tableName);
                }
            }];
        }
    }];
