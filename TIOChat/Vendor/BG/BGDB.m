    ...
    [self isExistWithTableName:tablename complete:^(BOOL isExist) {
        ...
        if (!isExist){//如果不存在就返回空
            ...
            [strongSelf queryWithTableName:tablename where:where complete:^(NSArray * _Nullable array) {
                ...
                END_TIME(t2, @"BGTool 数据转换完成")
                
//                [BGTool tansformDataFromSqlDataWithTableName:tablename class:cla array:array completion:^(NSArray *array) {
//                    END_TIME(t2, @"BGTool 数据转换完成")
...
-(void)queryObjectWithTableName:(NSString* _Nonnull)tablename class:(__unsafe_unretained _Nonnull Class)cla where:(NSString* _Nullable)where complete:(bg_complete_A)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    @autoreleasepool {
        [self queryObjectQueueWithTableName:tablename class:cla where:where complete:^(NSArray * _Nullable array) {
            complete(array);
            dispatch_semaphore_signal(self.semaphore);
        }];
    }
...
- (NSArray *)getAllColumsInTable:(NSString *)tableName ignoredkeys:(NSArray *)ignoredkeys cls:(Class  _Nullable __unsafe_unretained)cls
{
    __block NSArray *columNames = [NSArray array];
    [self executeDB:^(FMDatabase * _Nonnull db) {
        ...
        columNames = tempArrayM.count?tempArrayM:nil;
        
        if (!columNames.count) {
            NSMutableArray* keys = [NSMutableArray arrayWithArray:[BGTool getClassIvarList:cls Object:nil onlyKey:YES]];
            if (ignoredkeys) {
                [keys removeObjectsInArray:ignoredkeys];
            }
