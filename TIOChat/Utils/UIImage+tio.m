...
- (UIImage *)image_compressToByte:(NSUInteger)maxLength
{
    ...
    NSData *data = UIImageJPEGRepresentation(aImage, compression);
    if (data.length < maxLength) return aImage;
    
    CGFloat max = 1;
    CGFloat min = 0;
    for (int i = 0; i < 6; ++i) {
        compression = (max + min) / 2;
        data = UIImageJPEGRepresentation(aImage, compression);
