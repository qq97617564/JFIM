...
@interface TMineWalletViewController () <UITableViewDelegate, UITableViewDataSource>
...
@implementation TMineWalletViewController
...
- (void)requestData
{
    ...
    [TIOChat.shareSDK.walletManager fetchWalletDetailWithUid:self.uid walletid:self.walletid completion:^(TIOWallet * _Nullable wallet, NSError * _Nullable error) {
        ...
        self.blance = wallet.balance.integerValue/100.f;
        [self setMoney:[NSString stringWithFormat:@"%.2f",wallet.balance.integerValue/100.f]];
    }];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (selectedCell == self.wCell) { // 钱包明细
        [self.navigationController pushViewController:[WalletWaterListContainer.alloc init] animated:YES];
        ...
        CBWeakSelf
        [TIOChat.shareSDK.walletManager fetchBankCardListTokenWithUid:self.uid walletid:self.walletid completion:^(NSDictionary * _Nullable responseObject, NSError * _Nullable error) {
            CBStrongSelfElseReturn
            [self evoke:responseObject[@"token"] businessCode:EHKWEBOX_BUSINESSCODE_BANK];
        }];
    } else if (selectedCell == self.secCell){ // 安全设置
        CBWeakSelf
        [TIOChat.shareSDK.walletManager fetchSafeTokenWithUid:self.uid walletid:self.walletid completion:^(NSDictionary * _Nullable responseObject, NSError * _Nullable error) {
            CBStrongSelfElseReturn
            [self evoke:responseObject[@"token"] businessCode:EHKWEBOX_BUSINESSCODE_SETTING];
        }];
    } else if (selectedCell == self.accountCell) { // 账户信息
        [self.navigationController pushViewController:[WalletAccountInfoVC.alloc init] animated:YES];
    } else { // 帮助中心
        
    }
}

- (void)evoke:(NSString *)token businessCode:(EHKWEBOX_BUSINESSCODE)businessCode
{
    CBWeakSelf
    EHKWeboxManager * wallet = [EHKWeboxManager instanceManager];
    [utils configuration:wallet walletid:self.walletid token:token businessCode:businessCode vc:self];
