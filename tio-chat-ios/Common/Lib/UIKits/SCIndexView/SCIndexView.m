...
@property (nonatomic, strong) UIImageView *indicatorBg;
...
- (instancetype)initWithTableView:(UITableView *)tableView configuration:(SCIndexViewConfiguration *)configuration
{
    if (self = [super initWithFrame:tableView.frame]) {
        ...
        [self addSubview:self.indicatorBg];
...
- (void)showIndicator:(BOOL)animated
{
    ...
    if (self.configuration.indexViewStyle == SCIndexViewStyleDefault) {
        self.indicatorBg.center = CGPointMake(self.bounds.size.width - self.indicator.bounds.size.width / 2 - self.configuration.indicatorRightMargin, textLayer.position.y-self.configuration.indicatorHeight*0.5);
        self.indicator.center = self.indicatorBg.center;
    } else {
    ...
    if (animated) {
        ...
        self.indicatorBg.alpha = 0;
        self.indicatorBg.hidden = NO;
        [UIView animateWithDuration:kAnimationDuration animations:^{
            self.indicator.alpha = 1;
            self.indicatorBg.alpha = 1;
        }];
    } else {
        self.indicator.alpha = 1;
        self.indicator.hidden = NO;
...
- (UILabel *)indicator
{
    if (!_indicator) {
        ...
        _indicator.layer.backgroundColor = UIColor.clearColor.CGColor;
        ...
        switch (self.configuration.indexViewStyle) {
            ...
            {
                _indicator.bounds = self.indicatorBg.bounds;
            }
                break;
...
- (UIImageView *)indicatorBg
{
    if (!_indicatorBg) {
        _indicatorBg = [UIImageView.alloc initWithImage:[UIImage imageNamed:@"friend_bubble"]];
        _indicatorBg.bounds = CGRectMake(0, 0, self.configuration.indicatorHeight, self.configuration.indicatorHeight);
        _indicatorBg.hidden = YES;
    }
    return _indicatorBg;
}
