...
- (UIProgressView *)progressView {
    if (!_progressView){
        _progressView = [[UIProgressView alloc] initWithFrame:CGRectMake(0, Height_NavBar + 2, self.view.frame.size.width, 2)];
...
- (WKWebView *)webView{
    if(_webView == nil){
        ...
        _webView = [[WKWebView alloc] initWithFrame:CGRectMake(0, Height_NavBar, ScreenWidth(), ScreenHeight()) configuration:config];
        // UI代理
...
//        WKBackForwardList * backForwardList = [_webView backForwardList];
        ...
        if (self.autoUrl) {
            BOOL result = [[self.urlString substringToIndex:4] compare:@"http" options:NSCaseInsensitiveSearch];
            if (result) {
