...
- (NSArray<PPStickerMatchingResult *> *)matchingEmojiForString:(NSString *)string
{
    ...
    if (results && results.count) {
        ...
        for (NSTextCheckingResult *result in results) {
            ...
            NSString *emojiSubString = [showingDescription substringFromIndex:0];       // 去掉[
            emojiSubString = [emojiSubString substringWithRange:NSMakeRange(0, emojiSubString.length - 0)];    // 去掉]
